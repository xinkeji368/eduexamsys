...
import com.eduexamsys.entity.Message;
...
public interface MessageMapper {
    ...
    @Results({
            @Result(property = "replays", column = "temp_id",many = @Many(select = "com.eduexamsys.mapper.ReplayMapper.findAllById"))
    })
    IPage<Message> findAll(Page page);

    @Select("select id,title,content,time from message where id = #{id}")
    @Results({
            @Result(property = "replays", column = "id",many = @Many(select = "com.eduexamsys.mapper.ReplayMapper.findAllById"))
    })
